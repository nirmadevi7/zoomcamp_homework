...
model_ref = bentoml.sklearn.get("news_classification:2mddbyt5hcde5lg6")
dv = model_ref.custom_objects['CountVectorizer']
model_runner = model_ref.to_runner()

svc = bentoml.Service("News_classify", runners=[model_runner])

@svc.api(input=JSON(pydantic_model=Newstext), output=JSON())
async def classify(application_data):
    application_data=application_data.dict()
    print("application_data",application_data)
    application_data["title"] = text_preprocessing(application_data["title"])
    print(application_data["title"])

    vector = dv.transform([application_data["title"]])
    print(vector)
